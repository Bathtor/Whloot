...
class PathService extends Actor with ActorLogging {
    ...
    val routeCache = new LruMap[String, Route](50);
    ...
    private def addCache(start: Either[String, Int], end: Either[String, Int], route: Route) {
        routeCache += (pathReq2Key(start, end) -> route);
    }

    private def pathReq2Key(start: Either[String, Int], end: Either[String, Int]): String = {
        val firstPart = end match {
            case Left(name) => name;
            case Right(id)  => id.toString;
        }
        val secondPart = start match {
            case Left(name) => name;
            case Right(id)  => id.toString;
        }
